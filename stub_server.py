...
async def sc_info_no_audio(hub: Hub) -> None:
    ev = build_event(
        ...
        internal_log="DHCP handed a lease to HOST_C (vendor prefix matches a printer). No policy match; logged only.",
...
async def sc_low_rate_limit_auto(hub: Hub) -> None:
    ...
    ev = build_event(
        ...
        internal_log="HOST_A requested 212 distinct paths on the web server in 45s, 96% 404s. Signature http.scanner. rate_limit auto-applied.",
...
async def sc_high_block_ip_pending(hub: Hub) -> None:
    ...
    ev = build_event(
        ...
        internal_log="HOST_A attempted 47 SSH logins as USER_1 and USER_2 against bastion in 60s; all failed. Duration requested exceeds auto cap, approval required.",
...
async def sc_critical_isolate_pending(hub: Hub) -> None:
    ...
    ev = build_event(
        ...
        internal_log="HOST_B opened SMB sessions to 14 internal hosts in 40s using USER_3 credentials, 11 rejected. Consistent with lateral movement. Isolation proposed.",
...
async def sc_escalated_unreachable(hub: Hub) -> None:
    ev = build_event(
        ...
        internal_log="HOST_D issued 1,900 TXT queries for subdomains of one external zone in 60s. Signature not in catalog; novelty gate fired. Claude API unreachable, event left unreviewed.",
...
async def sc_audio_pending_then_ready(hub: Hub) -> None:
    ...
    ev = build_event(
        ...
        internal_log="HOST_A touched 60 distinct ports on the edge firewall over 10 minutes, one packet each. Watch added. TTS render still in flight.",
        tts_summary=tts,
        confidence=0.79,
...
async def sc_correlation_3_sources(hub: Hub) -> None:
    ev = build_event(
        ...
        internal_log="HOST_A, HOST_E and HOST_F each tried the same 6 usernames once against the VPN portal inside one window. Correlation gate fired (3 distinct sources). Claude verdict: coordinated spray, block all three.",
...
async def sc_revoke_session_pending(hub: Hub) -> None:
    ...
    ev = build_event(
        ...
        internal_log="USER_4 authenticated from HOST_A and HOST_G, geolocated to two continents, 4 minutes apart. Session revocation proposed.",
...
async def sc_burst(hub: Hub) -> None:
    ...
    for i, (sev, sig, title, tts) in enumerate(_BURST):
        ev = build_event(
            ...
            severity=sev,
            title=title,
            internal_log=f"Burst item {i + 1} of {len(_BURST)}: {title.lower()} involving HOST_{chr(65 + i)}. Exercising the phone's playback queue.",
            tts_summary=tts,
            confidence=0.8,
